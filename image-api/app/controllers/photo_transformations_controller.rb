class PhotoTransformationsController < ActionController::API
  def create
    if params[:format].present? && !PhotoTransformation::SUPPORTED_FORMATS.include?(params[:format])
    ...
    return redirect_to transformation_path(transformation) if transformation
